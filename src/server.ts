...
function registerLineTools(server: McpServer, contract: Contract, pricing: PricingConfig, client: RunApiClient): void {
  ...
  server.tool(
    ...
    `Look up RunAPI pricing for the ${META.lineSlug} model line.`,
    {
      ...
      action: endpointEnum.optional().describe("Endpoint name. Defaults to the line's primary endpoint.")
    },
    async ({ model, action }) => {
      const info = findModelForAction(service, action ?? endpoints[0], model, contract);
      if (!info) {
        return jsonText({ supported: false, message: "No matching model/endpoint in this model line." });
      }
